...
from database.db_session import database_instance
...
async def get_user_info(credentials: HAC = Security(security)):
    user_id = id_from_access_token(credentials)
    return db_commands.get_user_info(user_id)
...
@router.patch("userinfo/profile-picture", status_code=200)
async def update_profile_picture(picture_path: str, credentials: HAC = Security(security)):
    user_id = id_from_access_token(credentials)
    await database_instance.execute(
        db_commands.update_profile_picture(user_id, picture_path=picture_path)
    )
    return "Profile picture updated"

@router.patch("userinfo/location", status_code=200)
async def update_location(location_id: int, credentials: HAC = Security(security)):
    user_id = id_from_access_token(credentials)
    await database_instance.execute(
        db_commands.update_location(user_id, location_id=location_id)
    )
